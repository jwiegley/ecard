...
**Key Characteristics:**
- Each slot holds a **list of `ecard-property` objects** (enables multiple emails, phones, etc.)
- `:extended` slot uses alist structure: `((x-name . (list of ecard-property)) ...)`
...
(oref ecard email)  ; => List of ecard-property objects
;; Example: (#<ecard-property "EMAIL" "john@work.com" (("TYPE" . "work"))>
;;           #<ecard-property "EMAIL" "john@home.com" (("TYPE" . "home"))>)
```

**Extended Properties Alist:**
```elisp
;; X-* properties stored separately
(oref ecard extended)  ; => ((X-MANAGER . (#<ecard-property...>))
                        ;;     (X-DEPARTMENT . (#<ecard-property...>)))
```
...
|----------|---------|----------|
| `ecard-parse` | ecard object **or** list | Auto-detects single vs multiple |
| `ecard-parse-multiple` | **Always** list | Explicit multi-vCard handling |
| `ecard-parse-file` | ecard object or list | Parse from file |
| `ecard-parse-file-multiple` | Always list | Multi-vCard files |
| `ecard-parse-buffer` | ecard object or list | Parse current buffer |
| `ecard-parse-buffer-multiple` | Always list | Multiple vCards in buffer |
...
(let ((vc (ecard-parse-file "contact.vcf")))
  (if (listp vc)
    ...
    (message "Single: %s" (ecard-get-property-value vc 'fn))))

;; Multiple vCards guaranteed
(let ((vcs (ecard-parse-file-multiple "contacts.vcf")))
  (dolist (vc vcs)
    (process-contact vc)))
```
...
#### `ecard-create (&rest args)` (ecard.el:746-881)

Keyword-based vCard creation with automatic VERSION:4.0 injection:

```elisp
(ecard-create :fn "John Doe"                    ; Required
              :n '("Doe" "John" "Q." "Mr." "Jr.")
...
(ecard-get-property-values vc 'email)
...
(ecard-set-property vc 'email "new@example.com")
(ecard-set-property vc 'email "new@example.com" '(("TYPE" . "work")))

;; Add additional value (appends)
(ecard-add-property vc 'tel "+1-555-9999" '(("TYPE" . "cell") ("PREF" . "1")))

;; With group
(ecard-add-property vc 'email "alt@example.com" '(("TYPE" . "home")) "item1")
```

**Important for CardDAV:**
- Use `ecard-add-property` to preserve existing values
- Use `ecard-set-property` to replace (e.g., when syncing)
...
;; Write to file
(ecard-write-file vc "/path/to/contact.vcf")
```
...
Input Text
    ↓
ecard--unfold-lines (ecard.el:275-300)
    ↓ [List of logical lines]
ecard--parse-property-line (ecard.el:400-427)
    ↓ [Plist: :group :name :parameters :value]
ecard--unescape-value (ecard.el:302-322)
    ↓ [Unescaped values]
ecard--add-property-to-ecard (ecard.el:441-471)
    ↓ [ecard-property objects added to slots]
ecard--validate-ecard (ecard.el:562-600)
    ↓ [Validation checks]
ecard object
```
...
#### Line Unfolding (ecard.el:275-300)
...
#### Property Parsing (ecard.el:400-427)
...
#### Value Unescaping (ecard.el:302-322)
...
```elisp
;; Semicolon-separated components (ecard.el:416-418)
(member name '("N" "ADR" "ORG" "GENDER"))
;; => List: ("Doe" "John" "Q." "Mr." "Jr.")

;; Comma-separated text-list (ecard.el:420-422)
...
ecard object
    ↓
ecard--serialize-properties (ecard.el:525-532)
    ↓ [Iterate slots in defined order]
ecard--format-property (ecard.el:503-523)
    ↓ [Format single property]
ecard--escape-value (ecard.el:353-361)
    ↓ [Escape special characters]
ecard--fold-line (ecard.el:473-501)
...
Properties serialized in RFC 6350 recommended order (ecard.el:710-716):
```
...
```elisp
;; Read operations
(ecard-parse-file filename)           ; Uses insert-file-contents
(ecard-parse-buffer)                  ; Uses buffer-string

;; Write operations
(ecard-write-file vc filename)        ; Uses write-region
```
...
eieio-default-superclass
    ↓
ecard-property          (Single property with metadata)
ecard                   (Complete vCard object)
```
...
(oref vc email)  ; => List of ecard-property objects
```
...
```elisp
(ecard-parse text)      ; text → ecard object
(ecard-serialize vc)    ; ecard object → text
```
...
;; ... populate ...
;; Validate at end
(ecard--validate-ecard vc)  ; Signals errors
```
...
## 6. Integration Patterns (ecard-compat.el, ecard-org.el)

### 6.1 vCard 2.1/3.0 Compatibility Layer

**ecard-compat.el** provides backward compatibility:

```elisp
;; Auto-detect version and convert to 4.0
(ecard-compat-parse text)  ; Handles 2.1, 3.0, 4.0

;; Explicit version parsers
(ecard-compat-parse-21 text)
(ecard-compat-parse-30 text)
```
...
Legacy vCard Text
    ↓
ecard-compat--parse-legacy-property
    ↓
ecard-compat--convert-params-21/30
    ↓
ecard-compat--process-property-value (decode, charset convert)
    ↓
ecard-compat--build-ecard-40
    ↓
ecard 4.0 object
```
...
**ecard-org.el** provides bidirectional Org ↔ vCard conversion:
...
(ecard-org-ecard-to-entry vc 1)  ; level 1 heading

;; Import from .vcf file
(ecard-org-import-file "contacts.vcf")
```
...
- Keeps core ecard.el clean and focused
...
```
ecard-carddav.el          Core CardDAV protocol
ecard-carddav-sync.el     Synchronization engine
ecard-carddav-auth.el     Authentication (OAuth2, Basic)
ecard-carddav-cache.el    Local cache management
ecard-carddav-ui.el       User interface (optional)
```

### 7.3 Integration Points with ecard.el

#### 1. Parsing Server Responses

```elisp
;; CardDAV GET response
(let ((ecard-text (ecard-carddav--http-get url)))
  (ecard-parse ecard-text))  ; Returns ecard object
```
...
```elisp
;; Create new contact
(let ((vc (ecard-create :fn name :email email :tel phone)))
  (ecard-carddav--http-put url (ecard-serialize vc)))
```
...
```elisp
;; Fetch, modify, upload
(let ((vc (ecard-parse (ecard-carddav--http-get url))))
  (ecard-add-property vc 'email "new@example.com")
  (ecard-carddav--http-put url (ecard-serialize vc) etag))
```
...
(ecard-get-property-value vc 'uid)  ; Get UID
(ecard-set-property vc 'uid (generate-uid))  ; Set if missing
```
...
```elisp
(defun ecard-carddav-sync (addressbook-url)
  "Synchronize addressbook using sync-token."
  (let* ((sync-token (ecard-carddav-cache-get-sync-token addressbook-url))
         (changes (ecard-carddav--sync-collection addressbook-url sync-token)))
    ;; Process changes
    (dolist (change changes)
      (pcase (plist-get change :status)
        ('new (ecard-carddav-cache-add (plist-get change :ecard)))
        ('modified (ecard-carddav-cache-update (plist-get change :ecard)))
        ('deleted (ecard-carddav-cache-delete (plist-get change :href)))))
    ;; Update sync token
    (ecard-carddav-cache-set-sync-token addressbook-url
...
1. **Separation of concerns**
   ...
   - Use adapter pattern like ecard-compat.el and ecard-org.el
...
Follow ecard.el's comprehensive testing approach:
...
(ecard-carddav-discover-addressbooks "https://dav.example.com/principals/user@example.com/")
...
;; 2. Initial sync
(ecard-carddav-sync "https://dav.example.com/addressbooks/user@example.com/work/")
;; Fetches all contacts, stores locally with ETags

;; 3. Access local cache (ecard.el objects)
(let ((contacts (ecard-carddav-cache-list)))
  (dolist (vc contacts)
    (message "%s: %s"
...
(let ((vc (ecard-create :fn "New Contact"
                        :email "new@example.com"
                        :tel "+1-555-9999")))
  (ecard-carddav-create addressbook-url vc))

;; 5. Update existing contact
(let ((vc (ecard-carddav-cache-get-by-uid "uid-12345")))
  (ecard-add-property vc 'tel "+1-555-8888" '(("TYPE" . "cell")))
  (ecard-carddav-update addressbook-url vc))

;; 6. Incremental sync (only changed contacts)
(ecard-carddav-sync addressbook-url)  ; Uses sync-token
```
...
The ecard.el library provides a **solid foundation** for CardDAV integration:
...
- Create separate `ecard-carddav.el` module
- Follow adapter pattern from ecard-compat.el
